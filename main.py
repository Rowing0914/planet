...
def update_belief_and_act(args, env, planner, transition_model, encoder, belief, posterior_state, action, observation,
                          min_action=-inf, max_action=inf, explore=False):
    ...
    with torch.no_grad():
        ...
        for _ in tqdm(range(args.test_episodes)):
            ...
            belief, posterior_state, action = torch.zeros(1, args.belief_size, device=args.device), \
                torch.zeros(1, args.state_size, device=args.device), torch.zeros(1, env.action_size, device=args.device)
        ...
        if args.overshooting_kl_beta != 0:
            ...
            for t in range(1, args.chunk_size - 1):
                ...
                overshooting_vars.append((F.pad(actions[t:d], seq_pad), F.pad(nonterminals[t:d], seq_pad),
                                          F.pad(rewards[t:d], seq_pad[2:]), beliefs[t_], posterior_states[t_].detach(),
